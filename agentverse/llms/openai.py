...
def get_embedding(llm, text: str, attempts=3) -> np.array:
    try:
        ...
        if embedding_model is None or embedding_model.strip() == "":
            api_base = getattr(openai, "api_base", "") or os.environ.get("OPENAI_API_BASE", "")
            model_name = getattr(llm.args, "model", "") or ""
            # 针对不同提供方选择合适的兼容嵌入模型
            if getattr(openai, "api_type", "") == "azure":
                embedding_model = "text-embedding-ada-002"  # 由 Azure 的 deployment_id 使用
            elif "dashscope.aliyuncs.com" in str(api_base) or model_name.startswith("qwen") or "qwen" in model_name:
                # 通义千问 OpenAI 兼容模式默认使用 v4 嵌入
        ...
        if getattr(openai, "api_type", "") == "azure":
            embedding = openai.Embedding.create(
                input=[text], deployment_id=embedding_model
            )["data"][0]["embedding"]
        else:
            embedding = openai.Embedding.create(
